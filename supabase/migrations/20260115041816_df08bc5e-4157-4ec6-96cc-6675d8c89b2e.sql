-- Fix admin_bulk_delete_users function to remove confirm_deletion parameter
-- Error: "Could not find the function public.admin_bulk_delete_users(confirm_deletion, user_ids)"
-- This migration simplifies the function to only accept user_ids parameter
...
BEGIN
  ...
  IF user_ids IS NULL OR array_length(user_ids, 1) IS NULL THEN
    RAISE EXCEPTION 'user_ids parameter is required';
  END IF;
